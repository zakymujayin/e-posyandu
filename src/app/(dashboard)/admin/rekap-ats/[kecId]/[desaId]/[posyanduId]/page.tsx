...
export default async function ATSListAdminPage({ params }: { params: Promise<{ kecId: string; desaId: string; posyanduId: string }> }) {
  const session = await auth()
  if (!session?.user || session.user.role !== "ADMIN_DPMD") redirect("/login")

  const { kecId, desaId, posyanduId } = await params
  const posyandu = await prisma.posyandu.findFirst({ where: { id: posyanduId, desaId, desa: { kecamatanId: kecId } }, select: { name: true } })
  ...
  return (
    <PageContainer className="space-y-6">
      <PageHeader title={`ATS — ${posyandu.name}`} description="View only" backHref={`/admin/rekap-ats/${kecId}/${desaId}`} />
        ...
        {records.map((r) => (
          <TableRow key={r.id} className="hover:bg-muted/30">
            <TableCell className="px-4 py-3.5 font-semibold text-sm">
              <Link href={`/admin/rekap-ats/${kecId}/${desaId}/${posyanduId}/${r.id}`} className="text-primary hover:underline">{r.namaAnak}</Link>
            </TableCell>
            ...
            <TableCell className="px-4 py-3.5">
              <Button asChild variant="outline" size="sm" className="text-xs font-bold">
                <Link href={`/admin/rekap-ats/${kecId}/${desaId}/${posyanduId}/${r.id}`}>Detail</Link>
              </Button>
            </TableCell>
          </TableRow>
        ))}
