...
export default async function RekapATSDesaDrillPage({ params }: { params: Promise<{ desaId: string }> }) {
  ...
  const desa = await prisma.desa.findFirst({ where: { id: desaId, kecamatanId: petugas?.kecamatanId ?? "" }, select: { name: true } })
  if (!desa) notFound()
  ...
  return (
    <PageContainer className="space-y-6">
      <PageHeader title={`ATS — Desa ${desa.name}`} description="Per posyandu" backHref="/kecamatan/rekap-ats" />
        ...
        {rows.map((r) => (
          <TableRow key={r.posyanduId} className="hover:bg-muted/30">
            <TableCell className="px-4 py-3.5 font-semibold text-sm">
              <Link href={`/kecamatan/rekap-ats/${desaId}/${r.posyanduId}`} className="text-primary hover:underline">{r.posyanduName}</Link>
            </TableCell>
            ...
            <TableCell className="px-4 py-3.5">
              <Button asChild variant="outline" size="sm" className="text-xs font-bold">
                <Link href={`/kecamatan/rekap-ats/${desaId}/${r.posyanduId}`}>Lihat</Link>
              </Button>
            </TableCell>
          </TableRow>
        ))}
