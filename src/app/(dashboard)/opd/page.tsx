    ...
    <PageContainer className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex bg-muted/60 border border-border p-1 rounded-lg w-fit shadow-xs overflow-x-auto">
            {TABS.map((t, i) => (
              <Button
                key={t.value}
            ...
            {pengajuans.map((p) => (
              <TableRow key={p.id} className="transition-colors hover:bg-muted/30">
                <TableCell className="px-4 py-3.5 font-mono text-xs md:text-sm font-semibold">
                  <Link href={`/opd/tindak-lanjut/${p.id}`} className="text-primary hover:underline transition-colors">
                    {p.tiketNumber}
                  </Link>
                </TableCell>
