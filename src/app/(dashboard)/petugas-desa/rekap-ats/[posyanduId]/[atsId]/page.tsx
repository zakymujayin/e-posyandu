...
export default async function ATSDetailDesaPage({ params }: { params: Promise<{ posyanduId: string; atsId: string }> }) {
  const session = await auth()
  if (!session?.user || session.user.role !== "PETUGAS_DESA") redirect("/login")

  const { posyanduId, atsId } = await params
  ...
  const record = await prisma.anakTidakSekolah.findFirst({
    where: { id: atsId, posyanduId, posyandu: { desaId: petugas?.desaId ?? "" }, isActive: true },
  ...
  return (
    <PageContainer className="space-y-6">
      <PageHeader title={record.namaAnak} description={record.posyandu.name} backHref={`/petugas-desa/rekap-ats/${posyanduId}`} />
