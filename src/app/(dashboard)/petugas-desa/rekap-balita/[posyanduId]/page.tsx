...
export default async function RekapBalitaDesaPosyanduPage({
  ...
  if (!petugas?.desaId) {
    return (
      <PageContainer>
        <Alert variant="destructive"><AlertCircleIcon className="h-4 w-4" /><AlertDescription>Akun belum dihubungkan ke desa.</AlertDescription></Alert>
      </PageContainer>
    )
  }
  ...
  const posyandu = await prisma.posyandu.findUnique({
    where: { id: posyanduId },
    select: { name: true, desaId: true, desa: { select: { name: true } } },
  })
  if (!posyandu || posyandu.desaId !== petugas.desaId) redirect("/petugas-desa/rekap-balita")
          ...
          return (
            <TableRow key={b.id} className="hover:bg-muted/30 transition-colors">
              <TableCell className="px-4 py-3.5 font-semibold text-sm">
                <Link href={`/petugas-desa/rekap-balita/balita/${b.id}`} className="hover:text-blue-600 hover:underline transition-colors">
                  {b.namaBalita}
                </Link>
              </TableCell>
