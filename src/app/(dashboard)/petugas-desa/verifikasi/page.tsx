...
export default async function VerifikasiListPage({
  ...
  if (!petugasDesa?.desaId) redirect("/petugas-desa")
  ...
  const where: Record<string, unknown> = { desaId: petugasDesa.desaId }
  ...
  return (
    <PageContainer className="space-y-6">
      <PageHeader
        ...
        description="Pantau dan verifikasi seluruh pengajuan dari desa Anda."
      ...
      <div className="bg-card border border-border rounded-lg p-4 shadow-xs">
        <ClientFilterForm className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="flex flex-col gap-1.5">
            <FormLabel htmlFor="vd-status">Status</FormLabel>
            <select
              id="vd-status"
              name="status"
              defaultValue={status}
          ...
          <div className="flex flex-col gap-1.5">
            <FormLabel htmlFor="vd-dari">Dari Tanggal</FormLabel>
            <input
              id="vd-dari"
              type="date"
              name="dari"
          ...
          <div className="flex flex-col gap-1.5">
            <FormLabel htmlFor="vd-sampai">Sampai Tanggal</FormLabel>
            <input
              id="vd-sampai"
              type="date"
              name="sampai"
          ...
          <div className="sm:col-span-2 lg:col-span-4 flex gap-2 lg:items-end">
            <Button variant="outline" size="sm" className="font-bold text-xs md:text-sm" asChild>
              <Link href="/petugas-desa/verifikasi">Reset Filter</Link>
            </Button>
          </div>
        </ClientFilterForm>
      </div>
      ...
      {pengajuans.length === 0 ? (
        <EmptyState
          ...
          description="Coba ubah filter atau belum ada pengajuan yang masuk dari desa Anda."
        />
      ) : (
            ...
            {pengajuans.map((p) => (
              <TableRow key={p.id} className="transition-colors hover:bg-muted/30">
                <TableCell className="px-4 py-3.5">
                  <Link href={`/petugas-desa/verifikasi/${p.id}`} className="font-mono text-xs md:text-sm font-semibold text-primary hover:underline">
                    {p.tiketNumber}
                  </Link>
                </TableCell>
                ...
                <TableCell className="px-4 py-3">
                  <Button variant="outline" size="sm" asChild className="font-semibold text-xs h-8 rounded-lg">
                    <Link href={`/petugas-desa/verifikasi/${p.id}`}>Lihat</Link>
                  </Button>
                </TableCell>
              </TableRow>
            ))}
