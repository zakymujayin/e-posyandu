...
export async function POST(req: Request) {
  ...
  try {
    ...
    const existingCodes = new Set(
      (await prisma.posyandu.findMany({ select: { code: true } })).map((p) => p.code)
    )
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!row.desa_kode?.trim()) {
        results.push({ row: rowNum, status: "error", name, message: "desa_kode wajib diisi" })
        continue
      }
      ...
      const desa = await prisma.desa.findFirst({ where: { code: row.desa_kode.trim().toUpperCase() } })
      if (!desa) {
        results.push({ row: rowNum, status: "error", name, message: `desa_kode "${row.desa_kode}" tidak ditemukan` })
        continue
      }
